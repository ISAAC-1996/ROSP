...
generate_pharmacien_rows <- function(pharmacien_row) {
  rows <- paste0('
    <tr>
      <td style="border: 1px solid black;"><span style="color:blue; font-weight:bold">100 €</span> Pour la substitution d’un hybride <b>ET</b> d’un biosimilaire en 2024<i> (ajout du Ranibizumab – arrêté du 31 octobre 2024 sur la substitution des biosimilaires).</i></td>', generate_checkmark(pharmacien_row$SUBSTITU), generate_checkmark(pharmacien_row$SUBSTITU_12), '</tr>
    <tr>
...
send_email_to_pharmacien <- function(pharmacien_row, email, nom) {
  ...
  everys_paragraph <- if (pharmacien_row$everys == 1) {
    '<p style="color:black; font-weight:bold; margin-top: 15px;">Pour le logiciel Winpharma, nous attirons votre attention sur le fait, que les données transmises par ce LGO peuvent nous manquer par absence de remontées par le logiciel.</p>'
  } else {
    ''
  }
      ...
      <div style="position:relative; background-color:rgba(255, 255, 255, 0.9); padding:30px; border-radius:10px;">
          <div style="text-align:center; display:flex; justify-content:center; align-items:center; padding:40px 0;">
              <div style="background-color:#29a745; color:white; font-weight:bold; padding:25px 35px; border-radius:50px; font-size:24px; width:90%;">
                  Votre étude personnalisée sur les rémunérations exceptionnelles 2024 - 2<sup>ème</sup> évaluation
              </div>
          </div>
          <p>Bonjour,</p>
          ...
          <p>Ospharm, en octobre dernier, vous a communiqué une première évaluation des actes réalisés à fin septembre 2024 sous réserve de données complètes. Ce 2<sup>ème</sup> bilan vous offre une lecture instantanée des actions qui peuvent être réalisées d’ici le 31 décembre.</p>
          ...
          <table border="1" cellpadding="5" cellspacing="0" style="margin-left:5%; margin-right:5%; border-collapse:separate; border-radius:20px; border: 1px solid black; overflow:hidden;">
              <tr style="background-color:#29a745; border-radius: 15px;">
                  ...
                  <th style="text-align:center; color:white; border: 1px solid black;">RÉALISÉ À L\'ANNÉE *</th>
              </tr>
          ...
          <p><span style="font-weight:bold;">* </span> Les chiffres que nous communiquons sont issus des informations transmises depuis votre LGO à OSPHARM, votre dernière transmission date du : <span style="font-weight:bold;"> ', pharmacien_row$Vente, '</span>. Si des informations vous semblent erronées, n\'hésitez pas à prendre contact avec nos équipes.</p>
